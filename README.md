...
tests/             data (RLS), redclaw, flow, runtime, storefront
...
- **Feature gates.** `BILLING_ENABLED` and `PROVISIONING_ENABLED` default to false. FLOW 01 runs only with fake adapters and is disabled in production until real adapters pass acceptance.
...
2. Set env vars from `.env.example`. Server-only values (`DATABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `PAYMENT_WEBHOOK_SECRET`) must never get a `NEXT_PUBLIC_` prefix.
